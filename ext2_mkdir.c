...
The program should work like mkdir, creating the final directory on the specified
path on the disk. If any component on the path to the location where the 
final directory is to be created does not exist or if the spec fied 
directory already exists, then y-our program should return the appropriate 
error (ENOENT or EEXIST).

Note:

Please read the specifications to make sure you're implementing everything 
correctly (e.g., directory entries should be aligned to 4B, entry names are
not null-terminated, etc.).
When you allocate a new inode or data block, you *must use the next 
one available* from the corresponding bitmap (excluding reserved inodes, of course). Failure to do so will result in deductions, so please be careful about this requirement.
Be careful to consider trailing slashes in paths. These will show up d
uring testing so it's your responsibility to make your code as robust 
as possible by capturing corner cases.*/
unsigned char *disk;
...
int allocate_directory(int parent, char* path){
	...
	//while(parent_dblock->rec_len < 255 + 8 && read_count < EXT2_BLOCK_SIZE){
	//	read_count += parent_dblock->rec_len;
	//	parent_dblock = (struct ext2_dir_entry*)(disk + read_count + EXT2_BLOCK_SIZE * parent_inode->i_block[i - 1]);
	//} 
	/* Update the second from last dir_entry block's rec_len. */
	//read_count += 8 + parent_dblock->name_len + 4 - (parent_dblock->name_len)%4;
	
	
	//parent_dblock->rec_len = 8 + parent_dblock->name_len + 4 - (parent_dblock->name_len)%4;
	//parent_dblock = (struct ext2_dir_entry*)(disk + read_count + EXT2_BLOCK_SIZE * parent_inode->i_block[i - 1]);

	//if(EXT2_BLOCK_SIZE - read_count == 0){
	//	int new_parent_dblock = allocate_dblock();
	//	parent_inode->i_blocks += 2;
	//	parent_inode->i_block[i] = new_parent_dblock;
	//	parent_dblock = (struct ext2_dir_entry*)(disk + read_count + EXT2_BLOCK_SIZE * parent_inode->i_block[i]);
	//   read_count = 0;
	//}	
	//parent_dblock->inode = new_inode;
	//parent_dblock->rec_len = EXT2_BLOCK_SIZE - read_count;
	//parent_dblock->name_len = strlen(path);
    //parent_dblock->file_type = EXT2_FT_DIR;
	//strncpy(parent_dblock->name, path, parent_dblock->name_len);
...
int main(int argc, char **argv) {
	...
	if(parent == -1){
		printf("The path to the new directory does not exists.\n");
		return -ENOENT;
	}
	printf("Finish path check. \n");
	/* Check if all nodes are being used */
	if(check_space() == -1){
		printf("Very unlikely here, we are running out of space.\n");
		return -ENOSPC;
	}
    printf("Finish space check.\n");
	
	if(check_duplicate(path) != -1){
		printf("The directory already exists.\n");
		return -EEXIST;
	}
	printf("Finish duplicate check.\n");
