...
/*man without dream is no difference from fish without water*/
int dream_mark = 100;
...
int main(int argc, char **argv){
    ...
    char dest_without_last[tail+1];
    int length_of_last = 0;
    char alternative_filename[length_of_last+1];
    ...
    //printf("inode number for dest is: %d\n", cd_revised(dest,'d'));
    ...
    cp_inode->imode |= EXT2_S_IFREG;
    
    struct ext2_inode *dir_inode = (struct ext2_inode *)(inode_table[dir_inode_num-1]);    
    int block_index = -1;
    for(int i_block_index = 0; i_block_index < 12; i_block_index++){
    ...
    while(!entry_inserted){
		...
		cur_rec += cur_entry->rec_len;
            ...
            if(EXT2_BLOCK_SIZE - cur_rec < entry_cp->rec_len){
                if(i_block_index >= 11){
                    return ENOSPC;
                }else{
    ...
    while(!finish_copying){
        ...
        have_read = fread(buf, EXT2_BLOCK_SIZE, 1, fp);
        if(have_read > 0){
            buf[have_read] = '\0';
