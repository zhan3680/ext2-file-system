...
int delete_file(int del_file_inode, int parent_inode, char* path, char d_type) {
	int new_inode;
	...
	delete(parent_inode, del_file_inode, path, d_type);
...
int delete_directory(int del_dinode, int parent_inode, char* path) {
	int i, num_blocks, read_count;
    int is_last_entry = 1;
	i = 0;
	read_count = 0;
	...
	while(i < num_blocks){
		...
		while(cur_entry < num_entries){
		    /*if(strcmp(cur_dir_entry->name, ".") ||
			 strcmp(cur_dir_entry->name, "..") ){
				read_count += cur_dir_entry->rec_len;
		
				struct ext2_inode* parent_inode = &inode_table[cur_dir_entry->inode - 1];
				parent_inode->i_links_count -= 1;
				cur_dir_entry = (struct ext2_dir_entry*)(disk + delete_dblock*EXT2_BLOCK_SIZE + read_count);
				continue;
		
			}*/
			reach_end += cur_dir_entry->rec_len;
			while(reach_end < EXT2_BLOCK_SIZE){
			    cur_dir_entry = (struct ext2_dir_entry*)(disk + (delete_dblock)* EXT2_BLOCK_SIZE + reach_end);
			    reach_end += cur_dir_entry->rec_len;
			}
			reach_end = 0;
			//read_count = cur_dir_entry->rec_len;
			if(cur_dir_entry->file_type == EXT2_FT_DIR ){
				...
				if(strcmp(cur_dir_entry->name, ".") == 0 || strcmp(cur_dir_entry->name, "..") == 0){
				    delete_file(cur_dir_entry->inode, del_dinode, cur_dir_entry->name, 'd');	
				} else {
				    cur_dir_entry->name[cur_dir_entry->name_len] = '\0';
	                delete_directory(cur_dir_entry->inode, del_dinode, cur_dir_entry->name);   
				}
		    } else {
			    cur_dir_entry->name[cur_dir_entry->name_len] = '\0';
			    delete_file(cur_dir_entry->inode, del_dinode, cur_dir_entry->name, 'f');	
			}
			cur_entry += 1;
	...
	delete_file(del_dinode, parent_inode, path, 'd');
	return 0;
}
...
int main(int argc, char** argv){
	...
	if(argc == 4){
		...
		delete_directory(del_dinode, parent_inode, path);
		return 0;
	} else if(argc != 3){
		return -EINVAL;
	}
